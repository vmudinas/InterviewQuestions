namespace InterviewQuestions
{
    public class JumpCode
    {
        public int Jump(int[] nums)
        {
            ...
            var minJumpCount = 0;
            var i = 0;
            while (i < len - 1) // iterate from 0 to n-2 index
            {
                var nextJumpLimit = nums[i] + i;
                var nextJump = i + 1;
                for (int j = i + 1; j <= nums[i] + i && j < len; j++)
                {
                    // if next jump reachs last index and i = last index
                    // Otherwise next jump =
                    if (nextJumpLimit <= nums[j] + j || nums[j] + j >= len - 1)
                    {
                        nextJumpLimit = nums[j] + j;
                        nextJump = j;
                    }
                }
                minJumpCount++;
